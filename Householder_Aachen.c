...
int main(int argc, char* argv[]){
	...
	double complex *G1 = (double complex*) mkl_malloc(4*sizeof(double complex), 64);	// temporary matrix
	double complex *A2 = (double complex*) mkl_malloc(4*sizeof(double complex), 64);	// temporary matrix
	...
	for(k = 0; k < N; ++k){
		...
		if( idxj != k+1 ){
			...
			if(Nk/D == 0) mkl_nthreads = 1;
			mkl_set_num_threads(mkl_nthreads);
			zswap(&Nk, &G[k+1 + M*k], &M, &G[idxj + M*k], &M);
		}
