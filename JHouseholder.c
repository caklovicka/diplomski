...
int main(int argc, char* argv[]){
	...
	double complex *c = (double complex*) mkl_malloc(3 * 64 * omp_get_max_threads() * sizeof(double complex), 64);	// for the reflector, avoiding false sharing
		...
		//{
			...
			for(j = k+2; j < N; ++j){
				...
				*/
				...
				alpha = 1;
				beta = 0;
				zgemv(&trans, &Mk, &n, &alpha, &T[k], &M, &G[k+M*j], &inc, &beta, &c[3*64*nt], &inc);

				// g = g - 2E c
				alpha = -2;
				beta = 1;
				zgemv(&nontrans, &Mk, &n, &alpha, &E[k], &M, &c[3*64*nt], &inc, &beta, &G[k+M*j], &inc);
